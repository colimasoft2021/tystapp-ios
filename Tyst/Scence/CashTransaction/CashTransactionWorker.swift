...
class CashTransactionWorker {
    ...
    func deleteTransaction(request: DeleteTransaction.Request, completionHandler: @escaping ( _ message: String?, _ successCode: String?) -> Void) {
        NetworkService.dataRequest(with: ExpenseListAPIRouter.deleteTrans(request: request)) { (responce: WSResponse<DeleteTransaction.ViewModel>?, error: NetworkError?) in
            if let detail = responce {
                if  detail.arrayData != nil, let success = detail.setting?.isSuccess, let msg = detail.setting?.message, success {
                    completionHandler(msg, detail.setting?.success)
                } else {
